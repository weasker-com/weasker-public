...
const Hero = ({
  ...
  return (
    <div className="h-max sm:mx-0 sm:py-5 sm:pr-5 bg-white mb-2 rounded rounded-t-lg shadow max-w-[1000px] group w-full">
      <div className="flex flex-col items-start sm:items-center lg:flex-row gap-2 sm:gap-5 items-start sm:ml-2 lg:mx-auto max-w-[1000px] mr-auto p-3 sm:p-0 sm:px-5 rounded rounded-t-lg bg-white w-full">
        <div
          className={`flex flex-row gap-2 sm:gap-5 items-center ${
            cta && "sm:max-w-[70%]"
          }`}
        >
          {image && (
            <CldImage
              ...
              width={200}
              height={200}
              className="w-[50px] h-[50px] sm:w-11 sm:h-11 object-cover rounded-full border border-tl-dark-blue border-[2px] group-hover:border-tl-light-blue transition-border ease-in-out duration-300"
            />
          )}
